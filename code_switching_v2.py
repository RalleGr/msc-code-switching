...
# Get dictionaries
frequency_en_df = pd.read_csv(DICTIONARIES_PATH+'frequency_dict_en.csv',encoding='utf-16')
frequency_en_dict = frequency_en_df.set_index('word')['frequency'].to_dict()

frequency_es_df = pd.read_csv(DICTIONARIES_PATH+'frequency_dict_es.csv',encoding='utf-16')
frequency_es_dict = frequency_es_df.set_index('word')['frequency'].to_dict()

frequency_other_df = pd.read_csv(DICTIONARIES_PATH+'frequency_dict_other.csv',encoding='utf-16')
...
# Get data
filepath = 'bilingual-annotated/train.conll' # TODO use dev instead?
...
print(acc)
# 0.19602639591503074 # first try with unigrams and bigrams
